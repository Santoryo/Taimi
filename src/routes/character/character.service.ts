...
export async function updateCharacterInDb(apiKey: string, userId: string)
{
    ...
    for(const [index, character] of charactersData.entries())
    {
        ...
        const characterResponse = await db.insert(characters).values({ 
            userId: userId, 
            name: character.name,
            ...characterDetails
        }).onConflictDoUpdate({
        ...
        if(character.specializations.pve)
        {
            ...
            {
                await db.insert(specializationsPve)
                .values({
                    characterId: characterResponse[0].id,
                    specId: spec.id,
                    traits: spec.traits,
...
export async function getCharacterByName(name: string): Promise<FullCharacterDTO | null>
{
    ...
    const equipmentReq = await db
        .select()
        .from(equipment)
        .where(eq(equipment.characterId, character.id));
